...
RSpec.describe ChromaWave::Drawing::Primitives do
  ...
  describe '#draw_line' do
    ...
    it 'draws a single point when start equals end' do
      canvas.draw_line(5, 5, 5, 5, pen: pen_class.stroke(red))
      expect(canvas.get_pixel(5, 5)).to eq(red)
    end

    it 'supports thick lines' do
      canvas.draw_line(5, 10, 15, 10, pen: pen_class.stroke(black, width: 3))
    ...
    it 'raises ArgumentError when pen has fill' do
      expect { canvas.draw_line(0, 0, 5, 5, pen: pen_class.fill(red)) }
        .to raise_error(ArgumentError, /fill is not supported/)
    end

    it 'clips pixels outside canvas bounds' do
      expect { canvas.draw_line(-5, 10, 25, 10, pen: pen_class.stroke(black)) }.not_to raise_error
    end

    it 'returns self for chaining' do
      expect(canvas.draw_line(0, 0, 5, 5, pen: pen_class.stroke(black))).to equal(canvas)
    end
  end
  ...
  describe '#draw_polyline' do
    it 'draws connected segments' do
      points = [[0, 0], [5, 0], [5, 5]]
      canvas.draw_polyline(points, pen: pen_class.stroke(black))
    ...
    it 'connects last to first when closed' do
      points = [[0, 0], [5, 0], [5, 5]]
      canvas.draw_polyline(points, pen: pen_class.stroke(black), closed: true)
    ...
    it 'is a no-op for fewer than 2 points' do
      expect(canvas.draw_polyline([[1, 1]], pen: pen_class.stroke(black))).to equal(canvas)
    end
  end

  describe '#draw_rect' do
    it 'raises ArgumentError with pen having neither stroke nor fill' do
      expect { pen_class.new }
        .to raise_error(ArgumentError, /at least one of stroke: or fill:/)
    end

    context 'with fill only' do
      before { canvas.draw_rect(2, 3, 4, 3, pen: pen_class.fill(red)) }
    ...
    context 'with stroke only' do
      before { canvas.draw_rect(2, 3, 6, 4, pen: pen_class.stroke(black)) }
    ...
    context 'with both stroke and fill' do
      it 'draws fill then stroke (stroke on top)' do
        canvas.draw_rect(2, 2, 8, 6, pen: pen_class.new(stroke: black, fill: red))
    ...
    it 'is a no-op for zero dimensions' do
      canvas.draw_rect(5, 5, 0, 5, pen: pen_class.fill(red))
      expect(canvas.get_pixel(5, 5)).to eq(white)
    end

    it 'returns self for chaining' do
      expect(canvas.draw_rect(0, 0, 5, 5, pen: pen_class.fill(red))).to equal(canvas)
    end
  end
  ...
  describe '#draw_rounded_rect' do
    ...
    it 'fills a rounded rectangle' do
      canvas.draw_rounded_rect(2, 2, 12, 10, radius: 3, pen: pen_class.fill(red))
    ...
    it 'clamps radius to half the smaller dimension' do
      ...
      expect { canvas.draw_rounded_rect(2, 2, 6, 4, radius: 10, pen: pen_class.fill(red)) }
        .not_to raise_error
    end

    it 'returns self for chaining' do
      expect(canvas.draw_rounded_rect(0, 0, 10, 10, radius: 2, pen: pen_class.fill(red)))
        .to equal(canvas)
    end
  end
  ...
  describe '#draw_circle' do
    ...
    it 'draws a single pixel for radius 0' do
      canvas.draw_circle(10, 10, 0, pen: pen_class.fill(red))
      expect(canvas.get_pixel(10, 10)).to eq(red)
    end

    context 'with fill only' do
      before { canvas.draw_circle(10, 10, 4, pen: pen_class.fill(red)) }
    ...
    context 'with stroke only' do
      before { canvas.draw_circle(10, 10, 4, pen: pen_class.stroke(black)) }
    ...
    it 'supports thick stroke' do
      canvas.draw_circle(10, 10, 5, pen: pen_class.stroke(black, width: 3))
      # Should have colored pixels near the circumference
      expect(canvas.get_pixel(15, 10)).to eq(black)
    end

    it 'returns self for chaining' do
      expect(canvas.draw_circle(10, 10, 3, pen: pen_class.fill(red))).to equal(canvas)
    end

    it 'is a no-op for negative radius' do
      canvas.draw_circle(10, 10, -1, pen: pen_class.fill(red))
      expect(canvas.get_pixel(10, 10)).to eq(white)
    end
  end
  ...
  describe '#draw_ellipse' do
    ...
    context 'with fill only' do
      before { canvas.draw_ellipse(10, 10, 5, 3, pen: pen_class.fill(red)) }
    ...
    context 'with stroke only' do
      before { canvas.draw_ellipse(10, 10, 5, 3, pen: pen_class.stroke(black)) }
    ...
    it 'draws a single pixel for zero radii' do
      canvas.draw_ellipse(10, 10, 0, 0, pen: pen_class.fill(red))
      expect(canvas.get_pixel(10, 10)).to eq(red)
    end

    it 'returns self for chaining' do
      expect(canvas.draw_ellipse(10, 10, 3, 2, pen: pen_class.fill(red))).to equal(canvas)
    end
  end
  ...
  describe '#draw_arc' do
    it 'draws pixels along the arc' do
      ...
      canvas.draw_arc(10, 10, 5, 0, Math::PI / 2, pen: pen_class.stroke(black))
      expect(canvas.get_pixel(15, 10)).to eq(black) # 0 radians
    end

    it 'raises when pen has fill' do
      expect { canvas.draw_arc(10, 10, 5, 0, Math::PI, pen: pen_class.fill(red)) }
        .to raise_error(ArgumentError, /fill is not supported/)
    end

    it 'is a no-op for zero radius' do
      canvas.draw_arc(10, 10, 0, 0, Math::PI, pen: pen_class.stroke(black))
      expect(canvas.get_pixel(10, 10)).to eq(white)
    end

    it 'handles wrap-around (start > end after normalization)' do
      expect { canvas.draw_arc(10, 10, 5, -0.5, 0.5, pen: pen_class.stroke(black)) }
        .not_to raise_error
    end

    it 'returns self for chaining' do
      expect(canvas.draw_arc(10, 10, 5, 0, Math::PI, pen: pen_class.stroke(black)))
        .to equal(canvas)
    end
  end
  ...
  describe '#draw_polygon' do
    ...
    context 'with fill only' do
      ...
      before { canvas.draw_polygon(triangle, pen: pen_class.fill(red)) }
    ...
    context 'with stroke only' do
      ...
      before { canvas.draw_polygon(triangle, pen: pen_class.stroke(black)) }
    ...
    it 'is a no-op for fewer than 3 points' do
      expect(canvas.draw_polygon([[0, 0], [1, 1]], pen: pen_class.fill(red))).to equal(canvas)
    end

    it 'returns self for chaining' do
      expect(canvas.draw_polygon([[0, 0], [5, 0], [5, 5]], pen: pen_class.fill(red)))
        .to equal(canvas)
    end
  end
  ...
  describe '#flood_fill' do
    it 'fills a contiguous white region' do
      ...
      canvas.draw_rect(5, 5, 10, 10, pen: pen_class.stroke(red))
  ...
  describe 'works on Layer' do
    ...
    it 'draws within the layer region' do
      layer.draw_rect(0, 0, 5, 5, pen: pen_class.fill(red))
      expect(canvas.get_pixel(5, 5)).to eq(red)
      expect(canvas.get_pixel(9, 9)).to eq(red)
    end

    it 'clips at layer bounds' do
      layer.draw_rect(8, 8, 5, 5, pen: pen_class.fill(red))
  ...
  describe 'chaining multiple primitives' do
    it 'allows method chaining' do
      result = canvas
               .draw_rect(0, 0, 10, 10, pen: pen_class.fill(red))
               .draw_circle(5, 5, 3, pen: pen_class.stroke(black))
               .draw_line(0, 0, 19, 19, pen: pen_class.stroke(blue))
